...
class TKWindow(tk.Tk):
    ...
    widgets = {
        ...
        "label_a": {"self": "", "side": tk.LEFT, "text": "Spannungs Vector U in V:"},
        "label_z": {"self": "", "side": tk.LEFT, "text": "Widerstand Z in Ω:"},
        "label_theta": {
            "self": "",
            "side": tk.LEFT,
            "text": "Phasenanschnittwinkel ϑ in °:",
        },
        "entry_f": {"self": "", "side": tk.LEFT, "variable": "", "old_value": "0"},
        "entry_a": {"self": "", "side": tk.LEFT, "variable": "", "old_value": "0"},
        "entry_z": {"self": "", "side": tk.LEFT, "variable": "", "old_value": "0"},
        "scale_theta": {
            "self": "",
            "side": tk.LEFT,
            "variable": "",
    ...
    def create_widgets(self):
        ...
        self.label_info1 = tk.Label(
            self.frame_right_row_1,
            text="Darstellung von komplexen Zahlen:",
            font=("Arial", self.font_size),
            padx=30,
        ).pack(side=tk.LEFT)
        self.label_info2 = tk.Label(
            self.frame_right_row_2,
            text='Trennzeichen ist ein Punkt "."',
            font=("Arial", self.font_size),
            padx=30,
        ).pack(side=tk.LEFT)
        self.label_info3 = tk.Label(
            self.frame_right_row_3,
            text='Zeichen für komplexen Anteil ist "j"',
            font=("Arial", self.font_size),
            padx=30,
        ).pack(side=tk.LEFT)
        self.label_info4 = tk.Label(
            self.frame_right_row_4,
            text="Beispiel: -5.5+10j",
            font=("Arial", self.font_size),
            padx=30,
        ).pack(side=tk.LEFT)

        self.widgets["label_f"]["self"] = tk.Label(
            self.frame_left_row_1,
            text=self.widgets["label_f"]["text"],
            font=("Arial", self.font_size),
            padx=30,
            pady=10,
        )
        self.widgets["label_a"]["self"] = tk.Label(
            self.frame_left_row_2,
            text=self.widgets["label_a"]["text"],
            font=("Arial", self.font_size),
            padx=30,
            pady=10,
        )
        self.widgets["label_z"]["self"] = tk.Label(
            self.frame_left_row_3,
            text=self.widgets["label_z"]["text"],
            font=("Arial", self.font_size),
            padx=30,
            pady=10,
        )
        self.widgets["label_theta"]["self"] = tk.Label(
            self.frame_left_row_4,
            text=self.widgets["label_theta"]["text"],
            font=("Arial", self.font_size),
            padx=30,
            pady=10,
        )

        self.widgets["entry_f"]["self"] = tk.Entry(
            self.frame_left_row_1,
            textvariable=self.widgets["entry_f"]["variable"],
            font=("Arial", self.font_size),
        )
        self.widgets["entry_a"]["self"] = tk.Entry(
            self.frame_left_row_2,
            textvariable=self.widgets["entry_a"]["variable"],
            font=("Arial", self.font_size),
        )
        self.widgets["entry_z"]["self"] = tk.Entry(
            self.frame_left_row_3,
            textvariable=self.widgets["entry_z"]["variable"],
            font=("Arial", self.font_size),
        )
        self.widgets["scale_theta"]["self"] = tk.Scale(
            self.frame_left_row_4,
            showvalue=1,
            orient="horizontal",
            length=400,
    ...
    def init_axes(self):
        ...
        self.twin2.spines.right.set_position(
            ("axes", 1.2)
    ...
    def animate(self, i):
        ...
        if self.check_for_changed_input_values():
            ...
            self.axes.set_ylabel(self.figure_params["ylabel 1"])
            self.axes.set_ylim(
                -np.absolute(complex(self.widgets["entry_a"]["variable"].get())) * 1.1,
                np.absolute(complex(self.widgets["entry_a"]["variable"].get())) * 1.1,
            )
            self.axes.grid()
            self.axes.minorticks_on()
            ...
            self.twin1.set_ylim(
                -np.absolute(
                    complex(self.widgets["entry_a"]["variable"].get())
                    / complex(self.widgets["entry_z"]["variable"].get())
                )
                * 1.1,
                np.absolute(
                    complex(self.widgets["entry_a"]["variable"].get())
            ...
            self.twin2.set_ylabel(self.figure_params["ylabel 3"])
            max_power_value = np.max(
                [
                    np.max(self.graphs["p"]["curve"]),
            ...
            self.twin2.legend(
                handles=[plot_u, plot_i, plot_p, plot_S, plot_P, plot_Q],
                loc=self.figure_params["legend_position"],
                framealpha=1,
            )
            ...
            if "entry" in widget or "scale" in widget:
                if self.value_changed(widget):
                    ...
                    try:
                        ...
                        if "entry_f" in widget and float(new_value) < 10.0:
                            raise ValueError("f should be grater or equal to 10 Hz")
                        if "entry_z" in widget and type(new_value) != type(1.0):
                            self.widgets["scale_theta"]["variable"].set(0.0)
                        if (
                            "scale_theta" in widget
                            and new_value != 0.0
                            and np.imag(
                                complex(self.widgets["entry_z"]["variable"].get())
                            )
                            ...
                            self.widgets["entry_z"]["self"].insert(
                                0,
                                [f"{np.real(old_z)}" if np.real(old_z) != 0.0 else 1.0],
                            )
                    except ValueError:
    ...
    def value_changed(self, widget):
        ...
        """
        return (
            self.widgets[widget]["variable"].get() != self.widgets[widget]["old_value"]
        )
    ...
    def update_graphs(self):
        ...
        P = (
            ...
            * î
            ...
            / np.pi
            * (
                np.pi * (1 - np.absolute(theta) / 180)
                - np.sin(2 * np.pi * np.absolute(theta) / 180) / 2
            )
        )
        S = (
            û
        ...
        for t in self.graphs["t"]:
            ...
            u_complex = (
                û * (np.cos(wt + phi_u) + 1j * np.sin(wt + phi_u)) * amplitude_is_zero
            )
            i_complex = (
                î * (np.cos(wt + phi_i) + 1j * np.sin(wt + phi_i)) * amplitude_is_zero
            )
            ...
            self.graphs["S"]["curve"][int(t)] = (
                S if theta != 0.0 else np.absolute(S_complex)
            )  # = û*î/2
            self.graphs["P"]["curve"][int(t)] = (
                P if theta != 0.0 else np.real(S_complex)
            )
            self.graphs["Q"]["curve"][int(t)] = (
                Q if theta != 0.0 else np.imag(S_complex)
            )
    ...
    def phase_control(self, t, frequency, u_vector, theta):
        ...
        versatz_durch_phasenverschiebung = (
            T * np.angle(u_vector * 1j) / 2 / np.pi
        )  # +T/4
