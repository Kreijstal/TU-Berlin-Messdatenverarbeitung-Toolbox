...
class Switch(object):
    ...
    def add_case(self, value, callback, breaks=True):
        ...
        self.cases.append({"value": value, "callback": callback, "breaks": breaks})
...
class TKWindow(tk.Tk):
    ...
    def createWidgets(self):
        ...
        # Adding Labels
        tk.Label(self, text="Plot von der Spannung und Strom").grid(
    ...
    def window_loop(self):
        self.animation_object = animation.FuncAnimation(
            self.figure,
            self.animate,
            init_func=lambda: self.create_axes(1),
            interval=1,
            frames=itertools.count(step=self.tx),
        )
        self.mainloop()
    ...
    def animate(self, i):
        ...
        if "someFunction" in self.dictWidgets:
            ...
            for (Z, V) in zip(t, u):
                ...
                du = (V - (self.spannung[-1] if len(self.spannung) > 0 else 0)) / (
                    self.tx / 10
                )
            ...
            if len(self.spannung) > 1000:
                ...
                for (meas, ax, eff) in zip(
                    (self.spannung, self.current),
                ...
                self.axes[1].set_ylim(
                    ymin=min(0, min(self.power) * 1.01), ymax=max(0, maxpow * 1.01)
                )
            lnsa = []
            for (i, c, l, val, valeff) in zip(
                (0, 2, 1),
                ("blue", "orange", "red"),
                ("u", "i", "p"),
                (self.spannung, self.current, self.power),
                (self.ueff, self.ieff, self.peff),
            ):
                ax = self.axes[i]
                if show:
                    lnsa.append(
    ...
    def createSlider(self, name, from_, to, orient=tk.HORIZONTAL, **kwargs):
        self.dictWidgets[name] = tk.Scale(
            self, orient=orient, length=100, from_=from_, to=to, **kwargs
        )
    ...
    def init_window(self, title):
        ...
        self.createSlider("frequency", 0, 200, command=self.change_frequency).grid(
        ...
        self.createTextInput("Impedanz", "1").grid(
            column=4, row=2, sticky="w", ipadx=120
        )
        ...
        self.m.add_command(
            label="Kondensator", command=lambda: change_text_in_input("2*np.pi*f*u/du")
        )
        self.m.add_command(
            label="Kondensator + Widerstand",
            command=lambda: change_text_in_input("u/(du+u*99)*2*np.pi*f*2"),
        )
        self.m.add_command(
            label="Spule", command=lambda: change_text_in_input("u/(2*np.pi*f*U)")
        )
        self.m.add_command(
            label="Diode", command=lambda: change_text_in_input("9999999 if u<0 else 1")
        )
        self.m.add_command(
            label="DIAC",
            command=lambda: change_text_in_input(
                "99999999 if (du>0 and 9/10*a>u>0)or(du<0 and -9/10*a<u<0) else 1"
            ),
        )

        self.dictWidgets["Impedanz"].bind("<Button-3>", do_popup(self.m))
        CreateToolTip(
            self.dictWidgets["Impedanz"],
            """Impedanz als Funktion abhängig von Spannung und Zeit
Ein Python Ausdruck
...
f= Frequenz""",
        ...
        self.canvm.add_checkbutton(
            label="Effektive Werte zeigen", onvalue=1, offvalue=0, variable=self.showeff
        )
        self.canvm.add_checkbutton(
            label="Y Achse symmetrisch", onvalue=1, offvalue=0, variable=self.sym
        )
        self.canvm.add_checkbutton(
            label="Zeige Legende", onvalue=1, offvalue=0, variable=self.showlegend
        )
    ...
    def create_axes(self, view):
        ...
        for i, (l, c) in enumerate(
            [("Spannung (V)", "Blue"), ("Leistung (W)", "red"), ("Strom (A)", "orange")]
        ):
