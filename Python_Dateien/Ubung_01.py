...
class Animator:
    ...
    def __init__(self, anzahlVonBins, anzahlSamples, freiheitgradVonStudenTverteilung,legend,var1,var2,data):
        """
    ...
    def animate(self, i):
        ...
        leg = self.legend.get()
        if self.var1.get() == 0 and self.oldscale1 == newscale1 and self.oldscale2 == newscale2 and self.oldscale3 == newscale3:
            pass
        else:
            self.ax.cla()
            self.ax.set_title("Normal Verteilung")
            datemsatz1=np.random.normal(size=newscale2) if self.var2.get()==0 else self.data.zuffaligeDatenMitNormaleVerteilung[:newscale2]
            #print
            hist=self.ax.hist(datemsatz1,bins=newscale1,label="Histogram")
            self.ax.set_ylabel('Häufigkeit')

            self.ax.axvspan(datemsatz1.mean()-datemsatz1.std(),datemsatz1.mean()+datemsatz1.std(), ymax=hist[0].max() ,alpha=0.5, color='red',label="Vertrauensinterval 1$\sigma$, 68,3%")
            #ax.fill_between()
            if leg:
            ...
            datemsatz2=np.random.standard_t(newscale3,size=newscale2) if self.var2.get()==0 else self.data.zuffaligeDatenMitTstudentverteilung[:newscale2]
...
class GUI:
    ...
    def __init__(self):
        ...
        self.anzahlVonBins = Tk.Scale(self.root, orient=Tk.HORIZONTAL, length=300, from_=2, to=100)
        self.anzahlSamples = Tk.Scale(self.root, orient=Tk.HORIZONTAL, length=300, from_=2, to=10000)
        ...
        Tk.Label(self.root, text="Es ist möglich mit den Sliders zu Spielen").grid(column=0, row=1)
        ...
        self.var1 = Tk.IntVar()
        self.var2 = Tk.IntVar()
        self.IstRefreshSamplesGecheckht = Tk.Checkbutton(self.root, text='refresh samples/frame', variable=self.var1, onvalue=1, offvalue=0)
        self.istGefroren = Tk.Checkbutton(self.root, text='Freeze datensatz', variable=self.var2, onvalue=1, offvalue=0, command=self.generate_Datensatz)
        ...
        self.freiheitgradVonStudenTverteilung = Tk.Scale(self.root, orient=Tk.HORIZONTAL, length=300, from_=1, to=100)
        ...
        self.men.add_checkbutton(label="Zeige Legende", onvalue=1, offvalue=0, variable=self.legend)

        # Create an instance of the Animator class
        self.animator = Animator(self.anzahlVonBins, self.anzahlSamples, self.freiheitgradVonStudenTverteilung,self.legend,self.var1,self.var2,self.data)
        ...
        self.canvas.get_tk_widget().grid(column=0, row=2, columnspan=2, rowspan=7, padx=0, pady=0)
        ...
        self.ani = animation.FuncAnimation(self.animator.fig, self.animator.animate, np.arange(1, 64), interval=100, blit=False)
    ...
    def generate_Datensatz(self):
        ...
        if self.var2.get() == 1:
            ...
            self.data.zuffaligeDatenMitTstudentverteilung = np.random.standard_t(self.freiheitgradVonStudenTverteilung.get(), size=10000)
