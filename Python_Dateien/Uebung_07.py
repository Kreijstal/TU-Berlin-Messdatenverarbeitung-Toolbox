...
class TKWindow(tk.Tk):
    ...
    graphs = {
        "t": np.linspace(0, figure_params["bounds"], figure_params["bounds"] + 1),
        "u_0": {
    ...
    widgets = {
        ...
        "label_u0": {"self": "", "side": tk.LEFT, "text": "U Vektor in V:"},
        "label_z1": {"self": "", "side": tk.LEFT, "text": "Z_1 Vektor in Ω:"},
        "label_z2": {"self": "", "side": tk.LEFT, "text": "Z_2 Vektor in Ω:"},
        "label_z3": {"self": "", "side": tk.LEFT, "text": "Z_3 Vektor in Ω:"},
        "label_z4": {"self": "", "side": tk.LEFT, "text": "Z_4 Vektor in Ω:"},
        "entry_u0": {"self": "", "side": tk.RIGHT, "variable": "", "old_value": "0"},
        "entry_z1": {"self": "", "side": tk.RIGHT, "variable": "", "old_value": "0"},
        "entry_z2": {"self": "", "side": tk.RIGHT, "variable": "", "old_value": "0"},
        "entry_z3": {"self": "", "side": tk.RIGHT, "variable": "", "old_value": "0"},
        "entry_z4": {"self": "", "side": tk.RIGHT, "variable": "", "old_value": "0"},
    }

    measurement_bridge_values = {
        "u_0": 0.0 + 0.0j,
        "z_1": 0.0 + 0.0j,
        "z_2": 0.0 + 0.0j,
        "z_3": 0.0 + 0.0j,
    ...
    def init_params(self):
        ...
        self.widgets["picture"]["image"] = tk.PhotoImage(
            file=self.widgets["picture"]["path"]
        )
    ...
    def create_widgets(self):
        ...
        self.label_info1 = tk.Label(
            self.frame_row_2, text="Darstellung von komplexen Zahlen:", padx=30
        ).pack(side=tk.TOP)
        self.label_info2 = tk.Label(
            self.frame_row_2, text='Trennzeichen ist ein Punkt "."', padx=30
        ).pack(side=tk.TOP)
        self.label_info3 = tk.Label(
            self.frame_row_2, text='Zeichen für komplexen Anteil ist "j"', padx=30
        ).pack(side=tk.TOP)
        self.label_info4 = tk.Label(
            self.frame_row_2, text="Beispiel: -5.5+10j", padx=30
        ).pack(side=tk.TOP)

        self.widgets["label_u0"]["self"] = tk.Label(
            self.frame_row_1, text=self.widgets["label_u0"]["text"], padx=30, pady=10
        )
        self.widgets["label_z1"]["self"] = tk.Label(
            self.frame_row_3, text=self.widgets["label_z1"]["text"], padx=30, pady=10
        )
        self.widgets["label_z2"]["self"] = tk.Label(
            self.frame_row_4, text=self.widgets["label_z2"]["text"], padx=30, pady=10
        )
        self.widgets["label_z3"]["self"] = tk.Label(
            self.frame_row_5, text=self.widgets["label_z3"]["text"], padx=30, pady=10
        )
        self.widgets["label_z4"]["self"] = tk.Label(
            self.frame_row_6, text=self.widgets["label_z4"]["text"], padx=30, pady=10
        )

        self.widgets["entry_u0"]["self"] = tk.Entry(
            self.frame_row_1, textvariable=self.widgets["entry_u0"]["variable"]
        )
        self.widgets["entry_z1"]["self"] = tk.Entry(
            self.frame_row_3, textvariable=self.widgets["entry_z1"]["variable"]
        )
        self.widgets["entry_z2"]["self"] = tk.Entry(
            self.frame_row_4, textvariable=self.widgets["entry_z2"]["variable"]
        )
        self.widgets["entry_z3"]["self"] = tk.Entry(
            self.frame_row_5, textvariable=self.widgets["entry_z3"]["variable"]
        )
        self.widgets["entry_z4"]["self"] = tk.Entry(
            self.frame_row_6, textvariable=self.widgets["entry_z4"]["variable"]
        )

    def init_widgets(self):
        """Initialize widget variables after widget creation."""
        self.widgets["picture"]["self"].create_image(
            300, 160, image=self.widgets["picture"]["image"]
        )
    ...
    def place_widgets(self):
        ...
        [
            self.widgets[widget]["self"].pack(side=self.widgets[widget]["side"])
    ...
    def animate(self, i):
        ...
        if self.check_for_changed_entry():
            ...
            self.axes2.grid()
            self.axes2.minorticks_on()
            self.axes2.set_xbound(
                lower=-np.absolute(self.measurement_bridge_values["u_0"]),
                upper=np.absolute(self.measurement_bridge_values["u_0"]),
            )
            self.axes2.set_ybound(
                lower=-np.absolute(self.measurement_bridge_values["u_0"]),
                upper=np.absolute(self.measurement_bridge_values["u_0"]),
            )
            self.figure.tight_layout()
    ...
    def update_measurement_bridge_values(self):
        ...
        self.measurement_bridge_values["u_0"] = (
            complex(self.widgets["entry_u0"]["variable"].get())
            if self.widgets["entry_u0"]["variable"].get() != ""
            else 0.0 + 0.0j
        )
        self.measurement_bridge_values["z_1"] = (
            complex(self.widgets["entry_z1"]["variable"].get())
            if self.widgets["entry_z1"]["variable"].get() != ""
            else 0.0 + 0.0j
        )
        self.measurement_bridge_values["z_2"] = (
            complex(self.widgets["entry_z2"]["variable"].get())
            if self.widgets["entry_z2"]["variable"].get() != ""
            else 0.0 + 0.0j
        )
        self.measurement_bridge_values["z_3"] = (
            complex(self.widgets["entry_z3"]["variable"].get())
            if self.widgets["entry_z3"]["variable"].get() != ""
            else 0.0 + 0.0j
        )
        self.measurement_bridge_values["z_4"] = (
            complex(self.widgets["entry_z4"]["variable"].get())
            if self.widgets["entry_z4"]["variable"].get() != ""
            else 0.0 + 0.0j
        )
        ...
        self.measurement_bridge_values["i_12"] = (
            self.measurement_bridge_values["u_0"] / z_12
            if z_12 != 0.0 + 0.0j
            else 0.0 + 0.0j
        )
        self.measurement_bridge_values["i_34"] = (
            self.measurement_bridge_values["u_0"] / z_34
            if z_34 != 0.0 + 0.0j
            else 0.0 + 0.0j
        )

        self.measurement_bridge_values["u_1"] = (
            self.measurement_bridge_values["u_0"]
            - self.measurement_bridge_values["z_2"]
            * self.measurement_bridge_values["i_12"]
        )
        self.measurement_bridge_values["u_2"] = (
            self.measurement_bridge_values["u_0"]
            - self.measurement_bridge_values["z_1"]
            * self.measurement_bridge_values["i_12"]
        )
        self.measurement_bridge_values["u_3"] = (
            self.measurement_bridge_values["u_0"]
            - self.measurement_bridge_values["z_4"]
            * self.measurement_bridge_values["i_34"]
        )
        self.measurement_bridge_values["u_4"] = (
            self.measurement_bridge_values["u_0"]
            - self.measurement_bridge_values["z_3"]
            * self.measurement_bridge_values["i_34"]
        )

        self.measurement_bridge_values["u_b"] = (
            self.measurement_bridge_values["u_3"]
            - self.measurement_bridge_values["u_1"]
        )
    ...
    def update_graphs(self):
        ...
        for t in self.graphs["t"]:
            self.graphs["u_0"]["curve"][int(t)] = np.absolute(
                self.measurement_bridge_values["u_0"]
            ) * np.sin(
                2 * np.pi * self.f * t / 1000
                + np.angle(self.measurement_bridge_values["u_0"])
            )
            self.graphs["u_b"]["curve"][int(t)] = np.absolute(
                self.measurement_bridge_values["u_b"]
            ) * np.sin(
                2 * np.pi * self.f * t / 1000
                + np.angle(self.measurement_bridge_values["u_b"])
            )
    ...
    def plot_axes(self, axes, graph, x=None):
        ...
        """
        axes.plot(
            self.graphs["t"] if x is None else x["curve"],
            graph["curve"],
            label=graph["label"],
            color=graph["color"],
